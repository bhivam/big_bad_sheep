...
def calculateTStars(states: dict, size: int, grid):
    ...
    while True:
        for i in states:
            ...
            states[i] = newState
        if (abs(lossBetweenRounds/numStates) < 0.000001):
            ...
            break
        print(abs(lossBetweenRounds/numStates))
        lossBetweenRounds = 0
    ...
    def expectedTofSheepMove(state):
        ...
        if state[0] == state[1]:
            ...
            if deltaY >= 0 and onGrid(grid, state[1][0] + 1, state[1][1] + 0):
                possibleActionsOfSheep.append(
                    (state[1][0] + 1, state[1][1] + 0))
            if deltaY <= 0 and onGrid(grid, state[1][0] - 1, state[1][1] + 0):
                possibleActionsOfSheep.append(
                    (state[1][0] - 1, state[1][1] + 0))
        ...
        return sum([1/len(possibleActionsOfSheep) * states[(state[0], (i[0], i[1]))] for i in possibleActionsOfSheep])
...
grid, states = setupEnvironment(7)

f = open("storing_info7by7", "a")
for i in states:
...
numSheepToPen = 100000  # number of iterations of the simulation
...
for i in range(numSheepToPen):
    ...
    sheepPos = (Random().randint(0, len(grid) - 1),
                Random().randint(0, len(grid) - 1))
    while (grid[sheepPos[0]][sheepPos[1]]):
        sheepPos = (Random().randint(0, len(grid) - 1),
                    Random().randint(0, len(grid) - 1))

    dogPos = (0, 0)

    # finding minimum t state for a given random sheep position
    for initState in states:
        if initState[1] == sheepPos:
            if states[initState] < states[(dogPos, sheepPos)]:
                dogPos = initState[0]

    state = (dogPos, sheepPos)
    ...
    while (True):
        ...
        state = resultStates[0]

        for resultState in resultStates:
            if states[resultState] < states[state]:
                state = resultState
